			...
			#echo $F
   			CHECKWIFI=$(sudo grep -c "source=$F:name=wlan$G,channel_hop=true" $SITE)
				...
				echo 'source='$F':name=wlan'$G',channel_hop=true' | tee -a $SITE &>/dev/null
    				echo 'kismet_cap_linux_wifi --connect 127.0.0.1:3500 --tcp --fixed-gps 39.5,-75.5 --source=wlan'$F':name=Wifi'$G',type=linuxwifi,channel_hop=true &' &> $REMOTE
    			fi
		fi
	done
...
then
 	...
 	do
  		echo 'source=hci'$m':name=linuxbt'$m',type=linuxbluetooth' | tee -a $SITE &>/dev/null
    		echo 'kismet_cap_linux_bluetooth --connect 127.0.0.1:3500 --tcp --fixed-gps 39.5,-75.5 --source=hci'$m':name=hci'$m',type=linuxbt &' &> $REMOTE
  	done
fi
